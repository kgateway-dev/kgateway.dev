...
Kubernetes Gateway API is rapidly becoming the standard for networking in Kubernetes, with [more than two dozen projects](https://gateway-api.sigs.k8s.io/docs/implementations/list/) already supporting it. [Kgateway](https://kgateway.dev/) is a feature-rich, fast and battle-tested gateway that integrates seamlessly with Kubernetes. It extends the Gateway API to provide advanced features like rate limiting, timeouts, header manipulation, and request transformations.
