...
If you're using NGINX's cookie-based session affinity, this example shows how those annotations translate to an AgentGateway `BackendConfigPolicy`.
...
ingress2gateway print --providers=ingress-nginx --emitter=agentgateway \
  --input-file session-affinity-ingress.yaml > session-affinity-agentgateway.yaml
```
...
```bash
kubectl apply -f session-affinity-agentgateway.yaml
