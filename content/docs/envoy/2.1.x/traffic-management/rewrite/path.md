...
Use the [HTTPPathModifier](https://gateway-api.sigs.k8s.io/reference/api-spec/main/#httppathmodifiertype) to rewrite path prefixes. 
...
Use the [HTTPPathModifier](https://gateway-api.sigs.k8s.io/reference/api-spec/main/#httppathmodifiertype) to rewrite full paths. 
...
- **Prefix rewrite**: Use `ReplacePrefixMatch` in an `HTTPRoute` URLRewrite filter (see [Rewrite prefix path](#rewrite-prefix-path)).
- **Full path rewrite**: Use `ReplaceFullPath` in an `HTTPRoute` URLRewrite filter (see [Rewrite full path](#rewrite-full-path)).
