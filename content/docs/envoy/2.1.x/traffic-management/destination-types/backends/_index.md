...
Use a {{< gloss "Backends" >}}Backend{{< /gloss >}} resource to define a backing destination that you want kgateway to route to. A Backend destination is external to the cluster and, as such, cannot be represented as a Kubernetes Service. For more information, see the [Backend API docs]({{< link-hextra path="/reference/api/#backend" >}}). 
