...
1. The config and secret watcher components in the `{{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}}` pod watch the cluster for new Kubernetes Gateway API and {{< reuse "/kgw-docs/snippets/kgateway.md" >}} resources, such as Gateways, HTTPRoutes, or TrafficPolicies.
2. When the config or secret watcher detect new or updated resources, it sends the resource configuration to the {{< reuse "/kgw-docs/snippets/kgateway.md" >}} translation engine. 
