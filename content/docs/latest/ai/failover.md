...
In this example, you create a Backend with multiple pools for the same LLM provider. Each pool represents a specific model from the LLM provider. Failover priority is determined by the order in which the pools are listed in the Backend. The pool that is listed first is assigned the highest priority. For more information, see the [Priority Groups API reference docs]({{< link-hextra path="/docs/reference/api/#prioritygroups" >}}).
