...
| `saveBackendAddress` | `bool` | When this flag is set, the filter will add the resolved upstream address in the filter state. The state should be saved with key `envoy.stream.upstream_address` (See :repo:`upstream_address.h<source/common/stream_info/upstream_address.h>`). |
| `sslConfig` | [.gloo.solo.io.BackendSslConfig](../ssl.proto.sk/#upstreamsslconfig) | If provided, the autogenerated dynamic forward proxy cluster will use TLS when connecting to upstream hosts and verify the certificate chain. |
