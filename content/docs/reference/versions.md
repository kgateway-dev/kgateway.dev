...
Review the following information about supported release versions for kgateway OSS (open source).

## Supported versions

| Kgateway | Kubernetes | Kubernetes Gateway API | Envoy | Helm | Istio`†`|
|--------------|------------|-------|------|---------|---------|
...
New features for kgateway are developed on `main`. The latest version is released as a patch off of `main`.
