...
5. Create an Argo CD application to deploy the {{< reuse "/kgw-docs/snippets/kgateway.md" >}} CRD Helm chart. 
   ...
   kind: Application
   metadata:
     name: {{< reuse "/kgw-docs/snippets/helm-kgateway-crds.md" >}}-helm
     namespace: argocd
   spec:
     destination:
     ...
     project: default
     source:
       chart: {{< reuse "/kgw-docs/snippets/helm-kgateway-crds.md" >}}
       helm:
         skipCrds: false
       repoURL: {{< reuse "/kgw-docs/snippets/helm-path.md" >}}/charts
       targetRevision: {{< reuse "kgw-docs/versions/helm-version-flag.md" >}}
     syncPolicy:
       automated:
...
6. Create an Argo CD application to install the {{< reuse "/kgw-docs/snippets/kgateway.md" >}} Helm chart. For development builds, you might also need the parameter `controller.image.pullPolicy=Always` to ensure you get the latest image.
   ...
   kind: Application
   metadata:
     name: {{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}}-helm
     namespace: argocd
   spec:
     destination:
       namespace: {{< reuse "kgw-docs/snippets/namespace.md" >}}
     ...
     project: default
     source:
       chart: {{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}}
       helm:
         skipCrds: false
         parameters:
...
7. Verify that the control plane is up and running.
   ...
   {{< reuse "kgw-docs/snippets/pod-name.md" >}}-helm-6b5bb4db6b-c2pkq   1/1     Running   0          4m4s
   ```

8. Verify that the `{{< reuse "/kgw-docs/snippets/gatewayclass.md" >}}` GatewayClass is created. You can optionally take a look at how the gateway class is configured by adding the `-o yaml` option to your command.
