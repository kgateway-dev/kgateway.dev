...
3. Create a {{< reuse "kgw-docs/snippets/backend.md" >}} resource to define the Gemini destination.
...
4. Create an HTTPRoute resource to route requests to the Gemini {{< reuse "kgw-docs/snippets/backend.md" >}}. Note that {{< reuse "/kgw-docs/snippets/kgateway.md" >}} automatically rewrites the endpoint that you set up (such as `/gemini`) to the appropriate chat completion endpoint of the LLM provider for you, based on the LLM provider that you set up in the {{< reuse "kgw-docs/snippets/backend.md" >}} resource.
...
3. Create an {{< reuse "kgw-docs/snippets/backend.md" >}} resource to configure an LLM provider that references the AI API key secret.
   ...
   {{% reuse "kgw-docs/snippets/review-table.md" %}} For more information, see the [API reference]({{< link-hextra path="/reference/api/#aibackend" >}}).
...
4. Create an HTTPRoute resource that routes incoming traffic to the {{< reuse "kgw-docs/snippets/backend.md" >}}. The following example sets up a route on the `/openai` path to the {{< reuse "kgw-docs/snippets/backend.md" >}} that you previously created. The `URLRewrite` filter rewrites the path from `/openai` to the path of the API in the LLM provider that you want to use, `/v1/chat/completions`.
