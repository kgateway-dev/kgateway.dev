Use built-in tools to troubleshoot issues in your {{< reuse "/kgw-docs/snippets/kgateway.md" >}} setup.

{{< reuse "/kgw-docs/snippets/kgateway-capital.md" >}} consists of the control plane and an {{< reuse "/kgw-docs/snippets/data-plane.md" >}} data plane. If you experience issues in your environment, such as policies that are not applied or traffic that is not routed correctly, in a lot of cases, these errors can be observed at the proxy.
...
1. Enable port-forwarding on the control plane.
   ...
   ```sh
   kubectl port-forward deploy/{{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}} -n {{< reuse "kgw-docs/snippets/namespace.md" >}} 9095
   ```
...
3. Select one of the endpoints to continue debugging. {{< reuse "kgw-docs/snippets/review-table.md" >}} 
...
1. Create a {{< reuse "kgw-docs/snippets/gatewayparameters.md" >}} resource to add any custom settings to the gateway. For other settings, see the [{{< reuse "kgw-docs/snippets/gatewayparameters.md" >}} API docs]({{< link-hextra path="/reference/api/#gatewayparametersspec" >}}) or check out the [Gateway customization guides](../../setup/customize/).
...
3. Verify that a pod is created for your gateway proxy and that it has the pod settings that you defined in the {{< reuse "kgw-docs/snippets/gatewayparameters.md" >}} resource. 
   
   ```sh
   kubectl get pods -l app.kubernetes.io/name=debug-gateway -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -o yaml
   ```
...
5. Get the address of the debug gateway proxy.
   ...
   * **Cloud Provider LoadBalancer**
     ```sh
     export INGRESS_GW_ADDRESS=$(kubectl get svc -n {{< reuse "kgw-docs/snippets/namespace.md" >}} debug-gateway -o=jsonpath="{.status.loadBalancer.ingress[0]['hostname','ip']}")
     echo $INGRESS_GW_ADDRESS
     ```
   
   * **Port-forward for local testing**
     ```sh
     kubectl port-forward deployment/debug-gateway -n {{< reuse "kgw-docs/snippets/namespace.md" >}} 8080:8080
     ```
...
7. Review the debug logs for the gateway proxy.
   ...
   ```sh
   kubectl logs -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -l app.kubernetes.io/name=debug-gateway
   ```
...
As part of debugging, you might have noticed that your HTTPRoute or Gateway had an attached {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}. The {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}'s status might say `Accepted` and seem normal. However, when you checked the gateway configuration, the policy is not applied to the selected routes. Review the following common reasons for missing policies.

1. Verify that the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} is attached correctly. For example, you might use label selectors that do not match any HTTPRoute or Gateway. For more information, see [Policy attachment](../../about/policies/trafficpolicy/#policy-attachment-trafficpolicy).
...
3. Determine if you need a [Kubernetes ReferenceGrant](https://gateway-api.sigs.k8s.io/reference/api-types/referencegrant/). For example, the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} might rely on a GatewayExtension to enable a feature such as external auth. However, the GatewayExtension might be in a different namespace than the backing external auth service.
   ...
   * The external auth service is in the `{{< reuse "kgw-docs/snippets/namespace.md" >}}` namespace.
...
If you have not done yet, install the `{{< reuse "kgw-docs/snippets/cli-name.md" >}}` CLI. The `{{< reuse "kgw-docs/snippets/cli-name.md" >}}` CLI is a convenient tool that helps you gather important information about your gateway proxy. To install the `{{< reuse "kgw-docs/snippets/cli-name.md" >}}`, you run the following command: 
```sh
...
5. Check the proxy configuration that is served by the kgateway xDS server. When you create kgateway resources, these resources are translated into Envoy configuration and sent to the xDS server. If kgateway resources are configured correctly, the configuration must be included in the proxy configuration that is served by the xDS server. 
   ```sh
   {{< reuse "kgw-docs/snippets/cli-name.md" >}} proxy served-config --name http
   ```
...
6. Review the logs for each component. Each component logs the sync loops that it runs, such as syncing with various environment signals like the Kubernetes API. You can fetch the latest logs for all the components with the following command. 
   ```bash
   {{< reuse "kgw-docs/snippets/cli-name.md" >}} debug logs
   # save the logs to a file
   {{< reuse "kgw-docs/snippets/cli-name.md" >}} debug logs -f gloo.log
   # only print errors
   {{< reuse "kgw-docs/snippets/cli-name.md" >}} debug logs --errors-only
   ```
   
   You can use the `kubectl logs` command to view logs for individual components. 
   ```bash
   kubectl logs -f -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -l kgateway=kgateway
   ```
...
7. If you still cannot troubleshoot the issue, capture the logs and the state of kgateway in a file. 
   ```bash
   {{< reuse "kgw-docs/snippets/cli-name.md" >}} debug logs -f gloo-logs.log
   {{< reuse "kgw-docs/snippets/cli-name.md" >}} debug yaml -f gloo-yamls.yaml
   ```
   -->
