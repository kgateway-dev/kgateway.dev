Change the default route-level timeout of 15 seconds with an HTTPRoute or {{< reuse "kgw-docs/snippets/kgateway.md" >}} {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}. To ensure that your apps are available even if they are temporarily unavailable, you can use timeouts alongside [Retries]({{< link-hextra path="/resiliency/retry/" >}}).
...
1. Configure a timeout for a specific route by using the Kubernetes Gateway API-native configuration in an HTTPRoute or by using {{< reuse "kgw-docs/snippets/kgateway.md" >}}'s {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}. In the following example, you set a timeout of 20 seconds for httpbin's `/headers` path. However, no timeout is set along the `/anything` path. 
   ...
   3. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} with your timeout settings and use the `targetRefs.sectionName` to apply the timeout to a specific HTTPRoute rule. In this example, you apply the policy to the `timeout` rule that points to the `/headers` path in your HTTPRoute resource.
      ```yaml
      kubectl apply -f- <<EOF
