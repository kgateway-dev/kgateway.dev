...
You can configure different types of timeouts by using a Kubernetes Gateway API-native configuration, HTTPListenerPolicy, or a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} as shown in the following table.
...
| [Per-try timeout]({{< link-hextra path="/resiliency/retry/per-try-timeout" >}}) | Set a shorter timeout for retries than the overall request timeout.  | <ul><li>HTTPRoute</li><li>{{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} </li></ul>| <ul><li>HTTPRoute </li><li>HTTPRoute rule</li><li>Gateway listener ({{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} only)</li></ul> | {{% version include-if="2.4.x,2.5.x" %}}
| [TLS handshake timeout]({{< link-hextra path="/resiliency/timeouts/tls-handshake/" >}}) | Set a deadline for TLS handshake completion to protect the gateway from clients that open connections but never finish the handshake. | ListenerPolicy | Gateway |{{% /version %}}{{% version exclude-if="2.1.x,2.2.x,2.3.x,2.4.x" %}}
| [Maximum connection duration]({{< link-hextra path="/resiliency/timeouts/idle/#max-connection-duration" >}}) | Place an upper bound on the total lifetime of a connection, even when the connection is actively serving requests. | <ul><li>ListenerPolicy</li><li>BackendConfigPolicy</li></ul> | <ul><li>Gateway</li><li>Service</li></ul> |{{% /version %}}
