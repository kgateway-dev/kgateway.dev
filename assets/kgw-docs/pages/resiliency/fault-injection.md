...
1. Create or update the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to abort 100% of all requests with a 503 HTTP response code.
   ```yaml
   kubectl apply -f- <<EOF
   apiVersion: {{< reuse "kgw-docs/snippets/trafficpolicy-apiversion.md" >}}
   kind: {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}
...
```sh
kubectl delete {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} httpbin-fault -n httpbin 
```
...
When a gateway-level fault injection policy is in place, you can use `disable: {}` on a route-level {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to exempt specific routes from fault injection.
