...
Review how routes are sorted in the case of ties within a single HTTPRoute and across HTTPRoutes.
...
Even though one is more specific, {{< reuse "kgw-docs/snippets/kgateway.md" >}} does not automatically rank them by specificity. The pattern that wins depends on the list position (if within the same HTTPRoute), or creation timestamp and HTTPRoute name (if multiple HTTPRoutes). 
