...
1. [Install {{< reuse "/kgw-docs/snippets/kgateway.md" >}}]({{< link-hextra path="/quickstart/" >}}) in a cluster.
...
2. Create a Gateway resource with an HTTPS listener.
   ...
   | Setting | Description |
   |---------|-------------|
   | `spec.gatewayClassName` | The name of the Kubernetes GatewayClass. When you set up {{< reuse "/kgw-docs/snippets/kgateway.md" >}}, a default GatewayClass is set up for you. |
...
2. Check the status of the Gateway.
   ...
   ```bash
   kubectl get gateway grpc-gateway -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -o yaml
   ```
...
2. Verify that the GRPCRoute is applied successfully.
   ...
   ```bash
   kubectl get grpcroute grpc-echo-route -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -o yaml
   ```
...
1. Get the external address of the gateway and save it in an environment variable.
   ...
   ```bash
   export GATEWAY_IP=$(kubectl get gateway grpc-gateway -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -o jsonpath='{.status.addresses[0].value}')
   echo $GATEWAY_IP
   ```
   {{% /tab %}}
   {{% tab name="Port-forward for local testing" %}}
   ```bash
   kubectl port-forward svc/grpc-gateway -n {{< reuse "kgw-docs/snippets/namespace.md" >}} 8443:443
   ```
   {{% /tab %}}
   {{< /tabs >}}
   {{< /version >}}
   ...
   {{% tab name="Port-forward for local testing" %}}
   ```bash
   kubectl port-forward svc/grpc-gateway -n {{< reuse "kgw-docs/snippets/namespace.md" >}} 8080:8080
   ```
   {{% /tab %}}
   {{< /tabs >}}
   {{< /version >}}
