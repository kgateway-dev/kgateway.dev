...
1. Set up a header modifier that adds a `my-header: hello` request header. Choose between the HTTPRoute for a Gateway API-native way, or {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} for more [flexible attachment options](../../../about/policies/trafficpolicy/) such as a gateway-level policy.
   {{< tabs >}}
   {{% tab name="HTTPRoute" %}}
   ```yaml
   kubectl apply -f- <<EOF
   ...
   2. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} that adds a `my-header: hello` header to a request. The following example attaches the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to the http Gateway. 
      ```yaml
      kubectl apply -f- <<EOF
...
1. Set up a header modifier that sets a `my-header: hello` request header. Choose between the HTTPRoute for a Gateway API-native way, or {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} for more [flexible attachment options](../../../about/policies/trafficpolicy/) such as a gateway-level policy.
   {{< tabs >}}
   {{% tab name="HTTPRoute" %}}
   ```yaml
   kubectl apply -f- <<EOF
   ...
   2. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} that sets the `my-header` header value to `hello` on a request. The following example attaches the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to the http Gateway. 
      ```yaml
      kubectl apply -f- <<EOF
      apiVersion: {{< reuse "kgw-docs/snippets/trafficpolicy-apiversion.md" >}}
      kind: {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}
...
2. Set up a header modifier that removes the `User-Agent` header when requests are sent to the `headers.example` domain. Choose between the HTTPRoute for a Gateway API-native way, or {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} for more [flexible attachment options](../../../about/policies/trafficpolicy/) such as a gateway-level policy.
   ...
   2. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} that removes the `User-Agent` header from a request. The following example attaches the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to the http Gateway. 
      ```yaml
      kubectl apply -f- <<EOF
      apiVersion: {{< reuse "kgw-docs/snippets/trafficpolicy-apiversion.md" >}}
      kind: {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}
...
3. Create the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource that sets two request header values from keys that you stored in the `backend-creds` Secret. The following example attaches the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to the http Gateway.
   ```yaml
   kubectl apply -f- <<EOF
   apiVersion: {{< reuse "kgw-docs/snippets/trafficpolicy-apiversion.md" >}}
   kind: {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}
   ...
   |`headerModifiers.request.set.secretRef.namespace`|The namespace of the Secret. Optional. If `namespace` is omitted, it defaults to the namespace of the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}.|

4. Send a request to the httpbin app on the `headers.example` domain and confirm that the upstream sees the `X-Api-Key` and `X-Tenant-Id` headers with the values from the Secret. Note that the values do not appear in the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} or in the request you sent.
...
```sh
kubectl delete httproute httpbin-headers -n httpbin
kubectl delete {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} httpbin-secret-headers -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
kubectl delete secret backend-creds -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
```
...
1. Set up a header modifier that sets the `X-Client-Ip` header with the value of the downstream remote address. Choose between the HTTPRoute for a Gateway API-native way, or {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} for more [flexible attachment options](../../../about/policies/trafficpolicy/) such as a gateway-level policy. 
   {{< tabs >}}
   {{% tab name="HTTPRoute" %}}
   ```yaml
   kubectl apply -f- <<EOF
