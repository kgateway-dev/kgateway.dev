...
1. Set up a header modifier that adds a `my-response: hello` response header. Choose between the HTTPRoute for a Gateway API-native way, or {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} for more [flexible attachment options](../../../about/policies/trafficpolicy/) such as a gateway-level policy.
   {{< tabs >}}
   {{% tab name="HTTPRoute" %}}
   ```yaml
   kubectl apply -f- <<EOF
   ...
   2. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} that adds a `my-response: hello` header to a response. The following example attaches the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to the http Gateway. 
      ```yaml
      kubectl apply -f- <<EOF
...
1. Optional: Remove the resources that you created. 
   ...
   ```sh
   kubectl delete httproute httpbin-headers -n httpbin
   kubectl delete {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} httpbin-headers -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
   ```
   {{% /tab %}}
   {{< /tabs >}}
...
1. Set up a header modifier that sets a `my-response: custom` response header. Choose between the HTTPRoute for a Gateway API-native way, or {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} for more [flexible attachment options](../../../about/policies/trafficpolicy/) such as a gateway-level policy. 
   {{< tabs >}}
   {{% tab name="HTTPRoute" %}}
   ```yaml
   kubectl apply -f- <<EOF
   ...
   2. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} that sets the `my-response` header to a `custom` value on a response. The following example attaches the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to the http Gateway. 
      ```yaml
      kubectl apply -f- <<EOF
      apiVersion: {{< reuse "kgw-docs/snippets/trafficpolicy-apiversion.md" >}}
      kind: {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}
...
2. Set up a header modifier that removes the `content-length` header from the response. Choose between the HTTPRoute for a Gateway API-native way, or {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} for more [flexible attachment options](../../../about/policies/trafficpolicy/) such as a gateway-level policy. 
   {{< tabs >}}
   {{% tab name="HTTPRoute" %}}
   ```yaml
   kubectl apply -f- <<EOF
   ...
   2. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} that removes the `content-length` header from a response. The following example attaches the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to the http Gateway. 
      ```yaml
      kubectl apply -f- <<EOF
      apiVersion: {{< reuse "kgw-docs/snippets/trafficpolicy-apiversion.md" >}}
      kind: {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}
...
3. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} that sets a response header from the `response-signing` Secret. The following example attaches the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to the http Gateway.
   ```yaml
   kubectl apply -f- <<EOF
   apiVersion: {{< reuse "kgw-docs/snippets/trafficpolicy-apiversion.md" >}}
   kind: {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}
   ...
   The {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} `targetRefs` field does not accept a `namespace`. The policy must live in the same namespace as the resource it targets. To scope this example to a single HTTPRoute instead of the whole Gateway, change `kind` to `HTTPRoute` and `name` to the route's name, and create the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} (and the Secret) in the route's namespace.
   ...
   |`headerModifiers.response.set.secretRef.namespace`|The namespace of the Secret. Optional. If `namespace` is omitted, it defaults to the namespace of the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}. Cross-namespace references require a `ReferenceGrant`.  |
...
```sh
kubectl delete httproute httpbin-headers -n httpbin
kubectl delete {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} httpbin-secret-response-headers -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
kubectl delete secret response-signing -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
```
...
{{< reuse "kgw-docs/snippets/dynamic-req-resp-headers.md" >}}

1. Set up a header modifier that sets the `X-Response-Code` header with the value of the HTTP response code. Choose between the HTTPRoute for a Gateway API-native way, or {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} for more [flexible attachment options](../../../about/policies/trafficpolicy/) such as a gateway-level policy. 
   {{< tabs >}}
   {{% tab name="HTTPRoute" %}}
   ```yaml
   kubectl apply -f- <<EOF
