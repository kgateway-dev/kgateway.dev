...
For more information, see the [{{< reuse "kgw-docs/snippets/k8s-gateway-api-name.md" >}} documentation](https://gateway-api.sigs.k8s.io/reference/api-spec/main/spec/#httpurlrewritefilter).
...
Use a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to rewrite full paths with a regular expression (regex). The regex pattern and substitution that you define must follow the [RE2 syntax](https://github.com/google/re2/wiki/Syntax).
...
2. Create a policy to define your path rewrite rules by using regular expressions. The following {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} finds a path that matches the `^/anything/stores/(.*)/entities` pattern, such as: 
...
3. Send a request to the httpbin app along the `/anything/stores/us/entities` path that matches the regex pattern in your {{< reuse "/kgw-docs/snippets/trafficpolicy.md" >}}. Verify that you get back a 200 HTTP response code and that your request is rewritten to the `/anything/rewrite/path` path. 
   {{< tabs >}}
   {{% tab name="Cloud Provider LoadBalancer" %}}
   ```sh
...
3. Create a policy to define your path rewrite rules by using regular expressions. The following {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} finds a path that matches the `^/anything/stores/(.*)/entities` pattern, such as: 
   * **Allowed**: `/anything/stores/us/entities` or `/anything/stores/dummy/entities`
   * **Not allowed**: `/anything/stores/us/buildings` or `/get/stores/us/entities` 
...
kubectl delete {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} httpbin-rewrite-regex -n httpbin
