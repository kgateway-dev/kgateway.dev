...
| `get_cookie(cookie_name)` | Returns the value of the cookie with the given name from the request `Cookie` header. Cookie name matching is case-sensitive, per RFC 6265. Returns an empty string if the cookie is not present. |
| `get_cookie_i(cookie_name)` | Returns the value of the cookie with the given name from the request `Cookie` header, using a case-insensitive name match. Returns an empty string if the cookie is not present. |{{< /version >}}
