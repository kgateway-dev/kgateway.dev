In this guide you explore how to expose the {{< reuse "kgw-docs/snippets/kgateway.md" >}} proxy with an AWS network load balancer (NLB). The following use cases are covered:
...
1. Create a {{< reuse "kgw-docs/snippets/gatewayparameters.md" >}} resource with custom AWS annotations. These annotations instruct the AWS load balancer controller to expose the gateway proxy with a public-facing AWS NLB. 
   ```yaml
   kubectl apply -f- <<EOF
...
2. Depending on the annotations that you use on your gateway proxy, you can configure the NLB in different ways. 
   ...
   2. Create a Gateway with an HTTPS listener that terminates incoming TLS traffic. Make sure to reference the custom {{< reuse "kgw-docs/snippets/gatewayparameters.md" >}} resource and the Kubernetes secret that contains the TLS certificate information. 
      ```yaml
      kubectl apply -f- <<EOF
      apiVersion: gateway.networking.k8s.io/v1
      kind: Gateway
...
3. Verify that your gateway is created. 
   ```sh
   kubectl get gateway aws-cloud -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
   ```
   
4. Verify that the gateway service is exposed with an AWS NLB and assigned an AWS hostname. 
   ```sh
   kubectl get services aws-cloud -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
   ```
   
   Example output: 
   ```console
   NAME        TYPE           CLUSTER-IP      EXTERNAL-IP                                                                     PORT(S)        AGE
   aws-cloud   LoadBalancer   172.20.39.233   k8s-{{< reuse "kgw-docs/snippets/alb-elb-name.md" >}}-awscloud-edaaecf0c5-2d48d93624e3a4bf.elb.us-east-2.amazonaws.com   80:30565/TCP   12s
   ```
...
5. Review the NLB in the AWS EC2 dashboard. 
   ...
   3. Find and open the NLB that was created for you, with a name such as `k8s-{{< reuse "kgw-docs/snippets/alb-elb-name.md" >}}-awscloud-<hash>`.
...
2. Get the AWS hostname of the NLB. 
   ```sh
   export INGRESS_GW_ADDRESS=$(kubectl get svc -n {{< reuse "kgw-docs/snippets/namespace.md" >}} aws-cloud -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
...
2. Get the IP address that is associated with the NLB's AWS hostname. 
   ```sh
   export INGRESS_GW_HOSTNAME=$(kubectl get svc -n {{< reuse "kgw-docs/snippets/namespace.md" >}} aws-cloud -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
...
1. Delete the Ingress and Gateway resources.
   ```sh
   kubectl delete {{< reuse "kgw-docs/snippets/gatewayparameters.md" >}} custom-gw-params -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
   kubectl delete gateway aws-cloud -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
   kubectl delete httproute httpbin-elb -n httpbin
   kubectl delete secret https -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
   ```
