Use {{< reuse "/kgw-docs/snippets/kgateway.md" >}} as the ingress gateway to control and secure traffic that enters your {{< gloss "Service Mesh" >}}service mesh{{< /gloss >}}.
...
{{< reuse "/kgw-docs/snippets/kgateway-capital.md" >}} comes with an Istio integration that allows you to configure your gateway proxy with an Istio sidecar. The Istio sidecar uses mutual TLS (mTLS) to prove its identity and to secure the connection between your gateway and the services in your Istio service mesh. In addition, you can control and secure the traffic that enters the mesh by applying all the advanced routing, traffic management, security, resiliency, and AI capabilities that {{< reuse "/kgw-docs/snippets/kgateway.md" >}} offers. 

## About this guide

In this guide, you learn how to use {{< reuse "/kgw-docs/snippets/kgateway.md" >}} as an ingress gateway proxy for the workloads in your Istio {{< gloss "Service Mesh" >}}service mesh{{< /gloss >}}. You explore how to enable the Istio sidecar mesh integration in {{< reuse "/kgw-docs/snippets/kgateway.md" >}}, set up your ingress gateway proxy with a sidecar, and send secure mutual TLS traffic to the Bookinfo app as illustrated in the following image. 
...
Upgrade your {{< reuse "/kgw-docs/snippets/kgateway.md" >}} installation to enable the Istio integration. 
...
2. Verify that the gateway proxy is now successfully deployed. 
   ```sh
   kubectl get pods -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -l gateway.networking.k8s.io/gateway-name=http \
...
1. Delete the HTTPRoute and gateway-related resources. 
   ...
   kubectl delete gatewayparameters custom-gw-params -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
   ```
