...
{{< reuse "kgw-docs/snippets/setup-ambient-mesh.md" >}}
...
1. Enable the waypoint integration in your kgateway Helm chart. 
   ```sh
   helm get values {{< reuse "/kgw-docs/snippets/kgateway.md" >}} -n {{< reuse "/kgw-docs/snippets/namespace.md" >}} -o yaml > {{< reuse "/kgw-docs/snippets/kgateway.md" >}}.yaml
   
   helm upgrade -i --namespace kgateway-system --version v{{< reuse "kgw-docs/versions/patch-dev.md" >}} \
   kgateway oci://cr.kgateway.dev/kgateway-dev/charts/kgateway \
   --set controller.image.pullPolicy=Always \
   --set waypoint.enabled=true \
   --values {{< reuse "/kgw-docs/snippets/kgateway.md" >}}.yaml
   ```

2. Verify that you see the `{{< reuse "/kgw-docs/snippets/waypoint-class.md" >}}` GatewayClass in your cluster. 
   ```sh
...
Use the `{{< reuse "/kgw-docs/snippets/waypoint-class.md" >}}` GatewayClass to deploy {{< reuse "/kgw-docs/snippets/kgateway.md" >}} as a waypoint proxy in your cluster.
...
1. Create a waypoint proxy in the httpbin namespace. Note that creating a waypoint proxy does not automatically enforce Layer 7 policies for the apps in your cluster. To assign a waypoint, you must label your apps. You learn how to label your apps in a later step.
   ...
   kind: Gateway
   metadata:
     name: {{< reuse "/kgw-docs/snippets/waypoint-class.md" >}}
     namespace: httpbin
   spec:
     gatewayClassName: {{< reuse "/kgw-docs/snippets/waypoint-class.md" >}}
     listeners:
     - name: proxy
       port: 15088
...
2. Wait for the waypoint proxy to deploy successfully.
   ...
   ```sh
   kubectl -n httpbin rollout status deploy {{< reuse "/kgw-docs/snippets/waypoint-class.md" >}}
   ```
   
   Example output: 
   ```
   deployment "{{< reuse "/kgw-docs/snippets/waypoint-class.md" >}}" successfully rolled out
   ```

3. Label the httpbin2 and httpbin3 apps to use the waypoint proxy that you created.

   ```sh
   kubectl -n httpbin label svc httpbin2 istio.io/use-waypoint={{< reuse "/kgw-docs/snippets/waypoint-class.md" >}}
   kubectl -n httpbin label svc httpbin3 istio.io/use-waypoint={{< reuse "/kgw-docs/snippets/waypoint-class.md" >}}
   ```
...
Use {{< reuse "/kgw-docs/snippets/kgateway.md" >}}'s {{< reuse "/kgw-docs/snippets/trafficpolicy.md" >}} to apply a transformation policy to the httpbin2 app. 
...
2. Create a {{< reuse "/kgw-docs/snippets/trafficpolicy.md" >}} that applies a transformation policy to the httpbin2 app. In this example, the base64-encoded value from the `x-base64-encoded` header is decoded and added to the `x-base64-decoded` header, starting from the 11th character. 
   ```yaml
   kubectl apply -f- <<EOF
...
4. Optional: Remove the {{< reuse "/kgw-docs/snippets/trafficpolicy.md" >}} and HTTPRoute that you created in this guide. 
   ```sh
...
2. Create a {{< reuse "/kgw-docs/snippets/trafficpolicy.md" >}} that applies a local rate limiting policy to the httpbin2 app. In this example, only one request is allowed within 30 seconds. 
   ```yaml
   kubectl apply -f- <<EOF
   apiVersion: {{< reuse "/kgw-docs/snippets/trafficpolicy-apiversion.md" >}}
   kind: {{< reuse "/kgw-docs/snippets/trafficpolicy.md" >}}
...
4. Optional: Remove the {{< reuse "/kgw-docs/snippets/trafficpolicy.md" >}} and HTTPRoute. 
   ```sh
   kubectl delete {{< reuse "/kgw-docs/snippets/trafficpolicy.md" >}} ratelimit -n httpbin
   kubectl delete httproute httpbin2 -n httpbin
   ```
...
kubectl delete HTTPRoute httpbin2 -n httpbin
kubectl delete gateway {{< reuse "/kgw-docs/snippets/waypoint-class.md" >}} -n httpbin
