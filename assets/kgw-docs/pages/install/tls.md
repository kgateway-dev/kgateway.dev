...
3. Create a self-signed Certificate Authority (CA) that acts as the root of trust for the xDS gRPC server certificates. For production environments, replace the self-signed root issuer with your organization's CA issuer, such as Vault or a cloud CA service. For more information, see the [cert-manager CA issuer docs](https://cert-manager.io/docs/configuration/issuers/).
   ...
   kind: Issuer
   metadata:
     name: {{< reuse "kgw-docs/snippets/pod-name.md" >}}-xds-root-issuer
   ...
   kind: Certificate
   metadata:
     name: {{< reuse "kgw-docs/snippets/pod-name.md" >}}-xds-ca
     namespace: {{< reuse "kgw-docs/snippets/namespace.md" >}}
   spec:
     secretName: {{< reuse "kgw-docs/snippets/pod-name.md" >}}-xds-ca-secret
     issuerRef:
       name: {{< reuse "kgw-docs/snippets/pod-name.md" >}}-xds-root-issuer
       kind: Issuer
     isCA: true
     commonName: {{< reuse "kgw-docs/snippets/pod-name.md" >}}-xds-ca
     duration: 1h
     renewBefore: 5m
     privateKey:
       algorithm: RSA
...
4. Use the CA to sign a TLS certificate for the xDS gRPC server. This two-tiered approach keeps the root CA separate from the server certificate, and lets the server certificate be rotated independently of the CA. cert-manager automatically creates a Kubernetes secret with the required name `{{< reuse "kgw-docs/snippets/pod-name.md" >}}-xds-cert`, type `kubernetes.io/tls`, and the server certificate and private key in the `tls.crt` and `tls.key` keys.
   ...
   spec:
     ...
     isCA: false
     dnsNames:
     - {{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}}
     - {{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}}.{{< reuse "kgw-docs/snippets/namespace.md" >}}
     - {{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}}.{{< reuse "kgw-docs/snippets/namespace.md" >}}.svc
     - {{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}}.{{< reuse "kgw-docs/snippets/namespace.md" >}}.svc.cluster.local
     duration: 1h
     renewBefore: 5m
     privateKey:
       algorithm: RSA
...
Upgrade {{< reuse "kgw-docs/snippets/kgateway.md" >}} with TLS enabled for the controller. For complete steps, review the [Upgrade guide]({{< link-hextra path="/operations/upgrade" >}}).

1. Set your version of {{< reuse "/kgw-docs/snippets/kgateway.md" >}} in an environment variable, such as the latest patch version (`{{< reuse "kgw-docs/versions/n-patch.md" >}}`).
...
5. Confirm that the {{< reuse "/kgw-docs/snippets/kgateway.md" >}} control plane is up and running. 
...
1. Port-forward the control plane service on port 9977.
   ...
   ```sh
   kubectl port-forward -n {{< reuse "kgw-docs/snippets/namespace.md" >}} svc/{{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}} 9977
   ```
...
3. Port-forward the control plane deployment on port 9092.
   ...
   ```sh
   kubectl port-forward -n {{< reuse "kgw-docs/snippets/namespace.md" >}} deploy/{{< reuse "/kgw-docs/snippets/helm-kgateway.md" >}} 9092
   ```
