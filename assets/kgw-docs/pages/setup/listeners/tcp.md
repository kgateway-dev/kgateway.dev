...
1. Follow the [Get started guide]({{< link-hextra path="/quickstart" >}}) to install {{< reuse "kgw-docs/snippets/kgateway.md" >}}.

2. Install the experimental channel of the {{< reuse "kgw-docs/snippets/k8s-gateway-api-name.md" >}} so that you can use TCPRoutes.

   ```shell
   kubectl apply -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.3.0/experimental-install.yaml
   ```

3. {{< reuse "kgw-docs/snippets/prereq-listenerset.md" >}}

   **ListenerSets**: {{< reuse "kgw-docs/versions/warn-2-1-only.md" >}} Also, you must install the experimental channel of the Kubernetes Gateway API at version 1.3 or later.
...
1. Create a Gateway with a TCP listener. {{< reuse "kgw-docs/snippets/agw-gatewayclass-choice.md" >}}
   {{< tabs >}}
   ...
   1. Create a Gateway that enables the attachment of ListenerSets.
      ...
      |`spec.listeners`|{{< reuse "kgw-docs/snippets/generic-listener.md" >}} In this example, the generic listener is configured on port 80, which differs from port 8000 in the ListenerSet that you create later.|
...
2. Check the status of the Gateway to make sure that your configuration is accepted. Note that in the output, a `NoConflicts` status of `False` indicates that the Gateway is accepted and does not conflict with other Gateway configuration. 
   ```sh
   kubectl get gateway tcp-gateway -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -o yaml
   ```
...
2. Verify that the TCPRoute is applied successfully. 
   ...
   ```sh
   kubectl get tcproute/tcp-route-echo -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -o yaml
   ```
...
1. Get the external address of the gateway and save it in an environment variable.
   ...
   ```sh
   export INGRESS_GW_ADDRESS=$(kubectl get svc -n {{< reuse "kgw-docs/snippets/namespace.md" >}} tcp-gateway -o jsonpath="{.status.loadBalancer.ingress[0]['hostname','ip']}")
   echo $INGRESS_GW_ADDRESS   
   ```
   {{% /tab %}}
   {{% tab name="Port-forward for local testing" %}}
   ```sh
   kubectl port-forward deployment/tcp-gateway -n {{< reuse "kgw-docs/snippets/namespace.md" >}} 8000:8000
   ```
   {{% /tab %}}
   {{< /tabs >}}
