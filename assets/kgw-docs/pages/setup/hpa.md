You can bring your own Horizontal Pod Autoscaler (HPA) plug-in to {{< reuse "kgw-docs/snippets/kgateway.md" >}}. This way, you can automatically scale gateway proxy pods up and down based on certain thresholds, like memory and CPU consumption. 

To allow integration with HPA plug-ins, do not specify any custom replicas in the {{< reuse "kgw-docs/snippets/gatewayparameters.md" >}} resource. This way, your HPA plug-in can scale the proxy based on your autoscaling strategy. 
...
2. Review the metrics for the http Gateway. 
   ```sh
   kubectl top pod -n {{< reuse "kgw-docs/snippets/namespace.md" >}}  | grep http
   ```
...
```sh
kubectl delete hpa hpa -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
