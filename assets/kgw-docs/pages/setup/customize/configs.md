...
Replacing all volumes removes volumes generated by the control plane and can prevent the proxy from starting. A production replacement must re-declare every generated volume that the proxy requires. If you only need to add volumes, use `podTemplate.extraVolumes` instead.
{{< /callout >}}
