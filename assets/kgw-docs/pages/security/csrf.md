...
Use a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource to define your CSRF rules. 

1. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource to define your CSRF rules. The following example allows request from only the `allowThisOne.example.com` origin.
   ...
   {{< reuse "kgw-docs/snippets/review-table.md" >}}{{< version exclude-if="2.0.x" >}} For more information, see the [API docs]({{< link-hextra path="/reference/api/#csrfpolicy" >}}).{{< /version >}}
...
3. Send another request to the httpbin app. This time, you include the `allowThisOne.example.com` origin header. Verify that you get back a 200 HTTP response code, because the origin matches the origin that you specified in the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource.
...
1. Port-forward the gateway proxy. 
   ```sh
   kubectl port-forward -n {{< reuse "kgw-docs/snippets/namespace.md" >}} deploy/http 19000
   ```
...
```sh
kubectl delete {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} csrf -n httpbin
```
