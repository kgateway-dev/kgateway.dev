...
3. Verify that the auth server pod is running.
   ...
   ```sh
   kubectl get pods -n {{< reuse "kgw-docs/snippets/namespace.md" >}} -l app=ext-authz
   ```
...
2. Create a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} that applies passthrough auth at the Gateway level. For other common configuration examples, see [Other configuration examples](#other-configuration-examples).
