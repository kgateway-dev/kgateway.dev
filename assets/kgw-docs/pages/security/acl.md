...
The HTTP ACL filter evaluates every incoming HTTP request at Layer 7 against a set of IP-based rules and either allows or denies the request based on the client's source IP address (`source.address`). ACL rules are defined in the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource. 
...
You can mix `allow` and `deny` actions within the same {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource. When a client IP matches more than one rule, the most specific CIDR prefix always wins, regardless of the rule order that you specified in the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource. A `/32` single-host rule takes precedence over a `/16` subnet rule, which takes precedence over a `/8` range rule. With this capability, you can "punch holes" in to broader CIDR ranges. For example, you can deny an entire `10.0.0.0/8` range, while allowing a specific `10.1.0.0/16` subnet within that CIDR. For an example, see [Hole-punching with named rules](#hole-punching).

**Custom deny responses and surfacing the block reason**

By default, request are denied with a 403 HTTP response. You can customize the HTTP response code and add extra headers to a response. For an example, see [Custom deny response](#custom-deny-response-and-headers). 
...
The filter also increments the gateway proxy's `dev.kgateway.http.acl.blocked` metric on every denial. You can access the Envoy admin interface to monitor this metric. For more information, see [Monitor ACL blocks](#monitor-acl-denials).
...
Use a {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource to define your ACL rules.
...
1. Create the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource that denies all traffic by default and allows only requests from the `10.0.0.0/8` subnet. The policy targets the `httpbin` HTTPRoute that you set up as part of the prerequisites.
   ...
   {{< reuse "kgw-docs/snippets/review-table.md" >}} For more information, see the [API docs]({{< link-hextra path="/reference/api/#aclpolicy" >}}).
...
3. Log into the httpbin pod to run a curl request from within the cluster. By default, Kubernetes assigns pod IP addresses from the `10.0.0.0/8` range. Because this range matches the allow rule, you get back a 200 HTTP response.
   ...
   kubectl exec -n httpbin deploy/httpbin -c curl -- curl -vi \
     http://http.{{< reuse "kgw-docs/snippets/namespace.md" >}}.svc.cluster.local:8080/status/200 \
...
1. Update the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to allow all traffic by default and block IP addresses from [RFC 1918](https://datatracker.ietf.org/doc/html/rfc1918) private ranges. The `denyResponse.blockedByHeaderName` field adds a response header on every denial with the name of the matched rule.
...
1. Update the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to deny all traffic by default with a custom HTTP 451 status code and a `X-Blocked-Reason` response header.
...
1. Port-forward the gateway proxy to access the Envoy admin endpoint.
   ```sh
   kubectl port-forward deploy/http -n {{< reuse "kgw-docs/snippets/namespace.md" >}} 19000:19000
   ```
...
```sh
kubectl delete {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} acl-allowlist -n httpbin
```
