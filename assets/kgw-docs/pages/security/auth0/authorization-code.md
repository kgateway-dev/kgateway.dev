...
Create the Kubernetes Secret that holds the Auth0 client secret, a `GatewayExtension` that configures the provider, and a `TrafficPolicy` that enforces the flow on a route.
...
| `redirectURI` | The callback URL that kgateway sends to Auth0 as the `redirect_uri` parameter, and the path that the gateway intercepts to complete the code exchange. If you omit this field, it defaults to `<request-scheme>://<host>/oauth2/redirect` derived from the original request, which is easy to mismatch with the value registered in Auth0. Set it explicitly. |
...
| `endSessionEndpoint` | Handles single logout. When a user hits `/logout`, kgateway clears their session cookies and sends their browser to this URL so Auth0 ends the session too. This is RP-initiated logout in the OIDC spec. Only set it if `openid` is in your scopes. |
...
If your HTTPRoute uses a `PathPrefix` or `Exact` match, it must also match the OAuth2 callback path that you set in `redirectURI`. Otherwise, the redirect back from Auth0 returns a 404 error.
...
3. Log in with the test user credentials you created in the [Auth0 setup]({{< link-hextra path="/security/oauth/auth0/setup/#create-test-user" >}}).
...
If Auth0 shows `Invalid parameter: redirect_uri`, the `redirectURI` on the `GatewayExtension` does not match a redirect URI that is registered on the Auth0 application.

5. Optional: If you added the `denyRedirect` setting to your GatewayExtension, send the same request with `Accept: application/json`. Because `denyRedirect` matches on this header, the gateway returns `401` directly instead of redirecting.
...
This step is optional. By default, any unauthenticated request gets a `302` redirect to the Auth0 login page. That response works for a browser, but not for API clients. curl, mobile apps, and AJAX calls that hit an unauthenticated route silently follow the redirect, land on the Auth0 login HTML, and fail.
