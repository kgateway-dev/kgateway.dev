...
* **Route level** (optional): Override sampling rates, add route-specific span attributes, or disable tracing for individual routes by using the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource that targets an HTTPRoute or GRPCRoute.
...
* A listener-level `ListenerPolicy` with an OTel provider must be configured for any tracing to occur. Route-level `{{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}` tracing settings have no effect without it.
* Route-level tracing via `{{< reuse "kgw-docs/snippets/trafficpolicy.md" >}}` supports `HTTPRoute` and `GRPCRoute` targets only.
...
Deploy a minimal OpenTelemetry collector to receive and log trace data. Deploying it in the same namespace as the `ListenerPolicy` (`{{< reuse "kgw-docs/snippets/namespace.md" >}}`) avoids the need for a `ReferenceGrant`. For a production-grade setup with Grafana Tempo and persistent storage, see the [OpenTelemetry stack guide]({{< link-hextra path="/observability/otel-stack/" >}}).
...
```sh
kubectl get pod otel-collector -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
```
...
3. Check the OTel collector logs to verify that spans are being received.
   ```sh
   kubectl logs otel-collector -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
   ```
...
```sh
kubectl delete listenerpolicy tracing -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
kubectl delete pod otel-collector -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
kubectl delete service otel-collector -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
kubectl delete configmap otel-collector-conf -n {{< reuse "kgw-docs/snippets/namespace.md" >}}
```
...
Use the {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} resource to override sampling rates or add route-specific span attributes for individual HTTPRoutes or GRPCRoutes. Route-level attributes are merged with listener-level attributes; route-level values take priority on name collision.
...
When listener-level tracing is configured, use `disable: {}` on a route-level {{< reuse "kgw-docs/snippets/trafficpolicy.md" >}} to exempt specific routes from tracing. The `disable` field is mutually exclusive with all other `tracing` fields.
