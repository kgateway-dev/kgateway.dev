...
1. Deploy Grafana Loki to your cluster.
   ...
   --version {{< reuse "kgw-docs/versions/otel-stack-loki.md" >}} \
   --namespace telemetry \
   --create-namespace \
   --values - <<EOF
   loki:
...
2. Deploy Grafana Tempo to your cluster.
   ...
   --repo https://grafana.github.io/helm-charts \
   --version {{< reuse "kgw-docs/versions/otel-stack-tempo.md" >}} \
   --namespace telemetry \
   --create-namespace \
   --values - <<EOF
   persistence:
...
The OpenTelemetry collector acts as a centralized agent that scrapes metrics from the {{< reuse "/kgw-docs/snippets/kgateway.md" >}} control plane and data plane gateway proxies. Then, the OTel collector exposes these metrics in Prometheus format so that other tools in your observability stack, such as Grafana, can in turn scrape the OTel collector and visualize the data.
...
1. Deploy the metrics collector to handle numerical measurements and time-series data. Note that you can also use the `promexporter` endpoint with Prometheus to scrape metrics from the collector pod, if you prefer the `pull` model to the `push` model.
   ...
   --version {{< reuse "kgw-docs/versions/otel-stack-collector.md" >}} \
   --set mode=deployment \
...
1. Deploy Prometheus in your cluster.
   ...
   --version {{< reuse "kgw-docs/versions/otel-stack-prometheus.md" >}} \
   --namespace telemetry \
   --create-namespace \
   --values - <<EOF
   alertmanager:
