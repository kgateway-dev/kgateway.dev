...
1. **{{< reuse "docs/snippets/trafficpolicy.md" >}} with rateLimit.global**: Configure your rate limit policy in a {{< reuse "docs/snippets/trafficpolicy.md" >}}. The rate limit policy includes the descriptors to extract from requests for the Gateway to send to the Rate Limit Service.
...
3. **Rate Limit Service** - An external service that you set up to implement the Envoy Rate Limit protocol. The Rate Limit Service has the actual rate limit values to enforce on requests, based on the descriptors that the {{< reuse "docs/snippets/trafficpolicy.md" >}} includes.

### Response headers

When rate limiting is enabled, {{< reuse "docs/snippets/kgateway.md" >}} adds the following headers to responses. These headers help clients understand their current rate limit status and adapt their behavior accordingly.
...
| key | Required. The name for the descriptor to use when matching requests. Later, you use the descriptor key in the {{< reuse "docs/snippets/trafficpolicy.md" >}} to decide which rate limits to apply to requests. The Rate Limit Service expects one of the following values for the descriptor key: `remote_address` for a client IP address (`RemoteAddress` in the {{< reuse "docs/snippets/trafficpolicy.md" >}}), `path` for path matching (`Path` in the {{< reuse "docs/snippets/trafficpolicy.md" >}}), a key name for header matching (`Header` in the {{< reuse "docs/snippets/trafficpolicy.md" >}}), or a custom key name for matching on a generic key-value pair (`Generic` in the {{< reuse "docs/snippets/trafficpolicy.md" >}}). |
...
| rate_limit | Optional. The actual rate limit rule to apply. The example sets different rate limits for each descriptor key. If a descriptor key does not have a rate limit, the {{< reuse "docs/snippets/trafficpolicy.md" >}} cannot apply a rate limit to requests, and the requests that match the descriptor are allowed. |
...
1. Create a GatewayExtension.
   ...
   | domain | Domain identity for the rate limit service. If you have different domains for different teams, each team can create their own GatewayExtension that their own {{< reuse "docs/snippets/trafficpolicy.md" >}} can reference. | Yes |
...
2. Create a Kubernetes ReferenceGrant to allow the GatewayExtension to access the Rate Limit Service. Otherwise, you can create the GatewayExtension and {{< reuse "docs/snippets/trafficpolicy.md" >}} in the same namespace as the Rate Limit Service.
...
## Step 4: Create a {{< reuse "docs/snippets/trafficpolicy.md" >}} {#traffic-policy}

Create a {{< reuse "docs/snippets/trafficpolicy.md" >}} resource that applies rate limits to your routes. Note that the {{< reuse "docs/snippets/trafficpolicy.md" >}} must be in the same namespace as the GatewayExtension to select it.
...
Test the rate limits by sending requests to the Gateway. The following steps assume that you created the **client IP address** example {{< reuse "docs/snippets/trafficpolicy.md" >}}, which limits requests to 1 request per minute for a particular client IP address.
