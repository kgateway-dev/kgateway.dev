...
In kgateway, you use a [{{< version include-if="2.2.x,2.1.x" >}}{{< reuse "docs/snippets/trafficpolicy.md" >}}{{< /version >}}{{< version exclude-if="2.2.x,2.1.x" >}}TrafficPolicy{{< /version >}}]({{< link-hextra path="/about/policies/trafficpolicy/" >}}) to set up local rate limiting for your routes. You can choose between the following attachment options: 
