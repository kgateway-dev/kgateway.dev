...
TLS encryption is disabled by default. When enabled, the control plane mounts a `{{< reuse "docs/snippets/pod-name.md" >}}-xds-cert` TLS secret that you create and propagates the CA bundle to any {{< reuse "docs/snippets/pod-name.md" >}} data plane proxies to establish a secure connection. You might integrate your secret with a provider such as [cert-manager](https://cert-manager.io/docs/) to automate certificate management and rotation.
