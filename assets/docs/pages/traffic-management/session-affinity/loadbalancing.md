...
1. Create a BackendConfigPolicy to configure your load balancing algorithm for the httpbin app. 
   ...
   | `slowStart.aggression` | The rate of traffic increase over the duration of the slow start window. Defaults to 1.0, so that the endpoint receives a linearly-increasing amount of traffic.{{< version include-if="2.3.x,2.2.x,2.1.x" >}} For more information about fine-tuning this value, see the [API docs]({{< link-hextra path="/reference/api/#slowstart" >}}).{{< /version >}} |
