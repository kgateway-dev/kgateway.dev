...
2. Create a ListenerPolicy resource that sets `forwardClientCertDetails.mode: AlwaysForwardOnly`. This mode forwards the XFCC header for all requests.
   ...
   {{< reuse "docs/snippets/review-table.md" >}} For more information about the available fields, see the [API reference]({{< link-hextra path="/reference/api/#forwardclientcertdetails" >}}).
...
```sh
kubectl delete listenerpolicy forward-xfcc -n {{< reuse "docs/snippets/namespace.md" >}}
```
