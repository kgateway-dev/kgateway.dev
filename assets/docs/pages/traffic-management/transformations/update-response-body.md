...
* Use the `replace_with_random` function to replace specific patterns in the JSON body.{{< version exclude-if="2.0.x,2.1.x" >}} Note that with rustformation this function is evaluated at template compile time when its arguments are constants, so every request returns the same generated value. To get a fresh random per request, pass a per-request input such as `request_header("x-request-id")`. See [kgateway-dev/kgateway#13634](https://github.com/kgateway-dev/kgateway/issues/13634).{{< /version >}}
